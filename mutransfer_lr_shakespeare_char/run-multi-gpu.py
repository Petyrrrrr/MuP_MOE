#!/usr/bin/env python3
"""
Ultimate multi-GPU runner for Shakespeare character-level dataset.
Combines clean output processing with advanced monitoring features.
"""
...
class MultiGPURunner:
    ...
    def create_log_directory(self, base_path: str = "/home/ubuntu/MuP_MOE/std_out/mutransfer_lr_shakespeare_char") -> str:
    ...
    def build_command(self, config: Dict) -> Tuple[List[str], str]:
        ...
        out_dir = f"run_data/mutransfer_lr_shakespeare_char/out_{self.timestamp}/width{config['width']}_depth{config['depth']}_experts{config['num_exp']}_active{config['num_act']}_seed{config['seed']}_lr{config['lr']}"
        ...
        cmd_args = [
            ...
            "--block_size=1024",
            f"--n_layer={config['depth']}",  # depth=4 for Shakespeare
    ...
    def print_progress(self):
        ...
        print(f"\n{'='*60}")
        print(f"[Progress Report - Shakespeare Character Dataset]")
    ...
    def run(self):
        ...
        print(f"Starting Ultimate Multi-GPU Runner - Shakespeare Character Dataset")
        ...
        print(f"Total jobs: {len(configs)} (1 width × 4 num_exp × 10 lrs × 1 seed)")
        ...
        print(f"\n{'='*60}")
        print(f"ALL JOBS COMPLETED - SHAKESPEARE CHARACTER DATASET")
        print(f"{'='*60}")
        ...
        print(f"\nResults directory: run_data/mutransfer_lr_shakespeare_char/out_{self.timestamp}")
        print(f"Logs directory: {self.log_dir}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ultimate Multi-GPU Runner for Shakespeare Character Dataset")
